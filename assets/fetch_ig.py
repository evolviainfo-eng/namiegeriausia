...
def main():
    ...
    for e in edges:
        ...
        code = n['shortcode']
        name = f'ig-{code}'
        dest = os.path.join(IMG_DIR, f'{name}-560.webp')
        ...
        if not os.path.exists(dest):                 # only pull what's new
            ...
            for W in (560, 320):
                im.resize((W, W), Image.LANCZOS).save(
                    os.path.join(IMG_DIR, f'{name}-{W}.webp'), 'WEBP', quality=80, method=6)
            fetched += 1

        # tiny inline blur-up so a tile is never an empty square
        with Image.open(os.path.join(IMG_DIR, f'{name}-320.webp')) as t:
            b = io.BytesIO()
            t.convert('RGB').resize((16, 16), Image.LANCZOS).save(b, 'WEBP', quality=32, method=6)
            blur = base64.b64encode(b.getvalue()).decode()

        cap = n.get('edge_media_to_caption', {}).get('edges', [])
        cap = cap[0]['node']['text'] if cap else ''
        posts.append({
            'code': code,
            'url': f'https://www.instagram.com/p/{code}/',
            'ts': n.get('taken_at_timestamp'),
            'video': bool(n.get('is_video')),
            'alt': (cap.split('\n')[0][:110].strip() or 'Įrašas iš studijos kasdienybės'),
            'blur': blur,
        })
